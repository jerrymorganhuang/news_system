...
- 最前面先輸出：正面、負面或中性。
- 重點放在：最近發生了什麼、主要事件、市場關注重點，以及哪些資訊最可能影響公司基本面或市場預期。
- 依新聞的重要性與可信度整合內容，而非平均摘要所有新聞；相同事件請合併描述。
- Reuters、Bloomberg、公司公告、SEC 及 Tier-1 Sell-side 的內容應提高權重；若多位分析師提出相同觀點，請整合為市場共識，不需逐一列舉。
- 保留重要數字、產品、客戶、時程及管理層談話，不要自行推論新聞未提及的內容。
- 若新聞缺乏實質資訊、內容重複或與公司基本面關聯有限，請簡潔說明即可；若有重大事件，可適度增加篇幅至約400字。
- 直接進入重點，不要以「過去24小時」、「近期」等描述開場。
- 不要逐篇摘要、不要條列式，語氣保持客觀，使用繁體中文，風格如 sell-side analyst 的公司近況摘要。
- 一般長度約200–300字；重大事件可增加至約400字。
